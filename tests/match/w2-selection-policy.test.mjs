...
import { getSettingWeights, needsGenreRecovery, shouldSwapDuel, toggleDuelSide, toggleExclusiveSelection, toggleRankedSelection, validateSettingSelection } from "../../src/features/match/engine/selectionPolicy.mjs";
...
test("ranked choices preserve order, promote rank 2, and block a third", () => {
  let selected = toggleRankedSelection([], "a", 2);
  selected = toggleRankedSelection(selected, "b", 2);
  assert.deepEqual(toggleRankedSelection(selected, "c", 2), ["a", "b"]);
